...
import { Card, Image, Button, Switch, Input } from "antd";
...
function NFTComponent ({tokenId, stakedTokens, updateStakedNFT}) {
  ...
  useEffect(() => {
    setIsStaked(stakedTokens && stakedTokens.indexOf(tokenId) !== -1);
  }, [])
  ...
  const nftUnStaking = (tokenId) => {
    ...
    // Check unstaking to owner or winner
    if (toAddress) {
  ...
  return (
      ...
      cover={
        <>
...

          {
            ...
            <>
              <Switch
                checkedChildren="To Owner"
                unCheckedChildren="To Another"
                defaultChecked
                onChange={ () => {
              ...
              />
              {
                addInputVisible &&
                  <Input
                    placeholder="Input winner address"
                    onChange={(e) => {
    ...
    >
      ...
      <ModalCommon
        ...
        msg="Please input the valid winner address!"
      />
    </Card>
  );
}
